...
fclust_pred <- function(ui, k, userid, filmid, clusters, c, user = TRUE) {
  if (user == TRUE) {
  ...
  if (user == TRUE) {
    ...
    for (p in df_ind) {
      ...
      for (c in 1:n) {
        # prediction
        preds_i <- preds_i + clusters[c, userid] *
          fclust_pred(ui, k, userid, filmid, clusters, c)
      }
    ...
    for (p in df_ind) {
      ...
      for (c in 1:n) {
        # prediction
        preds_i <- preds_i + clusters[c, filmid] *
          fclust_pred(ui, k, userid, filmid, clusters, c, user)
      }
...
cval_mixed_fclust_nosim <- function(df, df_feat, t, n, m, k_range, clust_func,
                                    user = TRUE) {
  nk <- length(k_range)
  # initial scores table
  scores <- data.frame(rmse = rep(0, nk), mae = rep(0, nk), r2 = rep(0, nk),
